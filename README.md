...
| <a name="input_nat_gateway_availability_zones"></a> [nat\_gateway\_availability\_zones](#input\_nat\_gateway\_availability\_zones) | List of availability zones where a NAT gateway will be operating. If configured as `zonal`, one NAT GW will be created on public subnets deployed in the AZs listed here. If set up as `regional`, this is used to decide on how many AZs it will be expanding | `set(string)` | `[]` | no |
